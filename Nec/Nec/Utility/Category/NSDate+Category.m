...
+ (NSString *)timeWithTimeIntervalString:(NSString *)timeString
{
    ...
    NSString *str = [hourstr stringByAppendingString:minutestr];
    NSString *dateStr = [str stringByAppendingString:secondstr];
    return dateStr;
}

// 将用—样式隔开的时间转换成年月日样式的时间
+ (NSString *)dateStringwithString:(NSString *)string {
    NSString *dateText = [string stringByReplacingOccurrencesOfString:@"-" withString:@""];
    NSDateFormatter *inputFormatter= [[NSDateFormatter alloc] init];
    [inputFormatter setLocale:[[NSLocale alloc] initWithLocaleIdentifier:@"en_US"]];
    [inputFormatter setDateFormat:@"yyyyMMdd"];
    NSDate *inputDate = [inputFormatter dateFromString:dateText];
    NSDateFormatter *outputFormatter= [[NSDateFormatter alloc] init];
    [outputFormatter setLocale:[NSLocale currentLocale]];
    [outputFormatter setDateFormat:@"yyyy年MM月dd日"];
    dateText = [outputFormatter stringFromDate:inputDate];
    return dateText;
}
...
+ (NSString *)currentTimeDescription{
    ...
    NSString *currentTime = [dateFormatter stringFromDate:[NSDate date]];//当前年月日
    return currentTime;
}
...
+ (NSDate *)dateWithYear:(NSInteger)year andMonth:(NSInteger)month andDay:(NSInteger)day andHour:(NSInteger)hour andMiniute:(NSInteger)minute andSecond:(NSInteger)seconds{
    NSString * timerStr = [NSString stringWithFormat:@"%zd-%zd-%zd %zd:%zd:%zd",year,month,day,hour,minute,seconds];
    NSDateFormatter* formater = [[NSDateFormatter alloc] init];
    [formater setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
    [formater setTimeZone:[NSTimeZone timeZoneWithName:@"Asia/Shanghai"]];
    NSDate * date = [formater dateFromString:timerStr];
    return date;
}

+ (NSDate *)dateWithNowHourMinuteYear:(NSInteger)year andMonth:(NSInteger)month andDay:(NSInteger)day{
    NSDate *nowDate = [NSDate date];
    NSDate *retDate = [[self class] dateWithYear:year andMonth:month andDay:day andHour:nowDate.hour andMiniute:nowDate.minute andSecond:0];
    return retDate;
}
...
+ (id)YYMMddDateFormatter{
    return [self dateFormatterWithFormat:@"yyyy-MM-dd"];
}

+ (id)MMddHHmmssDateFormatter{
    return [self dateFormatterWithFormat:@"MM-dd HH:mm:ss"];
}
...
+ (NSString *)getShowTimeFromInterval:(NSTimeInterval)timeInterval dateFormatter:(NSDateFormatter *)dateFormatter
{
...
- (NSDate *)toiSO8601Date {
    ...
    NSDate *date = [ISO8601DateFormatter dateFromString:self];
    return date;
}

- (NSTimeInterval)toiSO8601DateInterval {
    return [[self toiSO8601Date] timeIntervalSince1970];
}
@end
