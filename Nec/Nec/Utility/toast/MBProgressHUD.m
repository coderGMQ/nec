...
- (void)show:(BOOL)animated{
    ...
    if(!_isNeedClock){
        //新增
        [(MBTimeProgressView *)indicator startAnimation];
    }
    
	useAnimation = animated;
	// If the grace time is set postpone the HUD display
	if(self.graceTime > 0.0){
        self.graceTimer = [NSTimer scheduledTimerWithAutoTimeInterval:self.graceTime
                                                               target:self
...
- (void)hide:(BOOL)animated{
	...
	if(self.minShowTime > 0.0 && showStarted){
		...
		if(interv < self.minShowTime) {
            self.minShowTimer = [NSTimer scheduledTimerWithAutoTimeInterval:(self.minShowTime - interv)
                                                                     target:self
...
- (void)animationFinished:(NSString *)animationID finished:(BOOL)finished context:(void*)context {
	[self done];
    if(!_isNeedClock){
        [(MBTimeProgressView *)indicator stopAnimation];
    }
}
...
//		[(UIActivityIndicatorView *)indicator startAnimating];
        ...
        //------------------2014.08.13 修改，实现时钟旋转效果------------------
        self.indicator = MB_AUTORELEASE([[MBTimeProgressView alloc] initWithFrame:CGRectMake(0, 0, 40, 40)]);
        //------------------2014.08.13 修改，实现时钟旋转效果------------------
...
//	}
    ...
    CGSize labelSize = [label.text boundingRectWithSize:CGSizeMake(280, 60)
